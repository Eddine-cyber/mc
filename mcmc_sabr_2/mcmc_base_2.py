...
from mcmc_sabr.utils import load_market_data, get_initial_params_from_calibration_all_maturities,  compute_prior_mean_std, compute_covariance_matrix_from_hessienne
from mcmc_sabr.diagnostics import calculate_r_hat
from mcmc_sabr_2.html_report_2 import save_results_to_html
...
class MCMCBase(ABC):
    """Classe de base abstraite pour les algorithmes MCMC SABR."""
    ...
    def setup(self):
        ...
        self.market_data_all_maturities = maturities, FwdImp, market_vols, strikes
        ...
        if self.adapt_cov :
            ...
            maturities, FwdImp, _, strikes = self.market_data_all_maturities
    ...
    def compute_log_likelihood(self, param_value):
        ...
        standard_likelihood = 0.0
        for i in range(len(maturities)):
            sabr_vols = vol_sabr_hagan(strikes, FwdImp[i], maturities[i], alpha, beta, rho, volvol)
            
            valid_mask = ~np.isnan(sabr_vols)
            residuals_squared = (market_vols[i][valid_mask] - sabr_vols[valid_mask])**2
        
            standard_likelihood += - np.sum(residuals_squared) 

        return standard_likelihood / (2 * config.OBSERVATION_ERROR**2)
